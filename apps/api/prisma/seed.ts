/**
 * Seed de démonstration STHECROH.
 * Crée des départements, programmes, un enseignant, des cours réels (avec modules/leçons),
 * et un examen — de quoi tester le parcours étudiant de bout en bout.
 *
 * Usage : pnpm --filter @sthecroh/api exec ts-node prisma/seed.ts
 * (ou ajouter "prisma": { "seed": "ts-node prisma/seed.ts" } dans package.json puis `pnpm prisma db seed`)
 */
...
async function main() {
  ...
  // ---------- Enseignants (avec intitulé public) ----------
  ...
  // ---------- Cours (correspondent à ceux affichés sur la page d'accueil) ----------
  const coursesData = [
  ...
  // ---------- Examen de démonstration sur le premier cours ----------
  ...
  if (!existingExam) {
    await prisma.exam.create({
      data: {
        ...
        maxScore: 100,
        availableTo: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000), // dans 5 jours
        instructions: 'Rédiger une dissertation de 1500 mots sur la doctrine de la grâce.',
      },
    });
  }

  // ---------- Quiz de démonstration avec de vraies questions QCM ----------
  ...
  // ---------- Étudiant de démonstration, déjà inscrit à 2 cours ----------
  ...
  // ---------- Certificat de démonstration pour l'étudiant ----------
  ...
  // ---------- Contenu de la page d'accueil (100% gérable depuis l'admin) ----------
  const testimonials = [
  ...
  for (const n of newsPosts) {
    ...
    if (!exists) await prisma.newsPost.create({ data: n });
  }
  ...
  for (const m of milestones) {
    ...
    if (!exists) await prisma.historyMilestone.create({ data: m });
  }
  ...
  // ---------- Message de démonstration (étudiant → enseignant) ----------
  ...
  // ---------- Bibliothèque numérique de démonstration ----------
  const libraryItems = [
  ...
  for (const item of libraryItems) {
    ...
    if (!exists) await prisma.libraryResource.create({ data: item });
  }
